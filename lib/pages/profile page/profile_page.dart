...
import 'package:fundora/pages/profile%20page/edit_profile_page.dart';
          ...
          TextButton(
            ...
            onPressed: () async {
              ...
              if (profileSnapshot.exists) {
                // Navigate to Edit Profile Page with current data
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => EditProfilePage(
                      currentProfileData: profileSnapshot.data() ?? {},
                    ),
                  ),
                );
              }
                ...
                CustomCard(
                  ...
                  children: [
                    TextSection(
                      title: "Problem",
                      content:
                          data['problem'] ?? 'No problem statement available',
                    ),
                    const SizedBox(height: 12),
                    TextSection(
                      title: "Solution",
                      content:
                          data['solution'] ?? 'No solution statement available',
                    ),
                  ],
                ),
                ...
                Card(
                  ...
                  child: ListTile(
                    leading: CircleAvatar(
                      ...
                      child: Text(
                        // Use the first letter of the founder's name, or 'F' if no name is available
                        (data['founder'] ?? 'Unknown Founder').isNotEmpty
                            ? (data['founder'] ?? 'F')[0].toUpperCase()
                            : 'F',
                        style:
                            const TextStyle(color: Colors.white, fontSize: 18),
                      ),
                    ),
                    title: Text(data['founder'] ?? 'Unknown Founder'),
